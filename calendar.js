...
//   day.units[]  { name, ratio, kids[], need[], merged[], shifts[], gaps[] }   (arrays are per 15-minute block)
...
(function () {
  ...
  function renderCalendar(el, day, opts) {
    ...
    const cols = day.units.map((u, ui) => {
      ...
      const { placed, count } = lanes(u.shifts);
      const w = 100 / count;
      const cards = placed.map(({ sh, lane }) => {
        const cls = ['cal-card', sh.borrowed ? 'moved' : 'home', sh.changed ? 'changed' : '', count > 2 ? 'narrow' : '', opts.picked === sh.who ? 'picked' : ''].join(' ');
      ...
      return `<div class="cal-col">
        <div class="cal-head"><strong>${esc(u.name)}</strong><span>1 teacher per ${u.ratio} children</span></div>
    ...
    el.innerHTML = `<div class="cal" style="--cols:${day.units.length}">
